...
public class FSBatchProcessCLI {
    ...
    public void usage() {
        ...
        helpFormatter.printHelp("org.apache.tika filesystem batch", options);
    }
    ...
    private TikaInputStream getConfigInputStream(String[] args, boolean logDefault) throws IOException {
        ...
        if (batchConfigFile != null) {
            ...
            if (logDefault) {
                LOG.info("No config file set via -bc, relying on org.apache.tika-app-batch-config.xml or default-org.apache.tika-batch-config.xml");
            }
            //test to see if there's a org.apache.tika-app-batch-config.xml on the path
            URL config = FSBatchProcessCLI.class.getResource("/org.apache.tika-app-batch-config.xml");
            if (config != null) {
                is = TikaInputStream.get(
                        FSBatchProcessCLI.class.getResourceAsStream("/org.apache.tika-app-batch-config.xml"));
            } else {
                is = TikaInputStream.get(
                        FSBatchProcessCLI.class.getResourceAsStream("default-org.apache.tika-batch-config.xml"));
            }
        }
        return is;
    }
